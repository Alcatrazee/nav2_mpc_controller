  ...
  const nav2_costmap_2d::Costmap2D * costmap) const
{
  ...
  for (auto & bound : corridor.bounds) {
    ...
    // 全截面扫描：在 [-default_right_width, +default_left_width] 提取所有连续无障碍通行区间
    std::vector<std::pair<double, double>> free_intervals;
    bool in_free = false;
    double start_d = -config_.default_right_width;
    ...
    for (double d = -config_.default_right_width; d <= config_.default_left_width + 1e-4; d += step_size) {
      ...
      if (costmap->worldToMap(test_x, test_y, mx, my)) {
        ...
        if (cost >= config_.costmap_cost_threshold) {
          is_free = false;
        }
      } else {
        is_free = false;
      ...
      if (is_free) {
        ...
        if (in_free) {
          in_free = false;
          free_intervals.push_back({start_d, d - step_size});
        }
      }
    }
    if (in_free) {
      free_intervals.push_back({start_d, config_.default_left_width});
    }
    ...
    // 智能选取最优通行区间：优先与上一截面走廊保持空间连续性
    std::pair<double, double> best_interval = free_intervals.front();
    double min_dist_to_prev = 1e9;
    ...
    for (const auto & interval : free_intervals) {
      ...
      double center_d = 0.5 * (interval.first + interval.second);
      double dist = std::abs(center_d - prev_center_d);
    ...
    prev_center_d = 0.5 * (bound.d_min + bound.d_max);
  }
}
  ...
  const rclcpp::Time & stamp)
{
  ...
  // 1. Initial raycasting against costmap obstacles
  initializeWithCostmap(corridor, costmap);
  ...
  // 当接近参考轨迹终点最后 0.8m 进近阶段，将走廊平滑漏斗化收拢到终点目标容差 (±0.05m)，强制将避障偏移车辆平滑“漏”入终点坐标
  if (!corridor.bounds.empty()) {
    double total_corridor_s = corridor.bounds.back().s;
    double approach_dist = 0.80;
    double terminal_d_tol = 0.05;

    for (auto & bound : corridor.bounds) {
      double s_rem = total_corridor_s - bound.s;
      if (s_rem < approach_dist && s_rem >= 0.0) {
